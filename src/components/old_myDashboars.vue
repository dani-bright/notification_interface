<template>
  <div>
    <input type="date" @change="LoadAllData" id="date" :value="date"> 
    <div id="mainContainer"> 
      ...
      <div class="holder">
        <div class="loader" v-show="loader">
          ...
          <li v-for="(data,index) in accounts" :key='index'>
            {{data}}
            <div class="tool">
                <div class="alertNumber">5</div>
                <button v-on:click="ViewMessageFromTopic" :data-topic="data">voir</button>
            </div>
            </li>
      </div>
...
export default { 
  ...
  methods: {
      ...
      array.forEach(item=>{
        ...
        .then(response => {
          if(response.request.status==200){
              ...
              this.getMessages(this.allMessages);
              requestDone++;
    ...
    ViewMessageFromTopic(e) {
        console.dir(e.target.getAttribute("data-topic")); 
